...
const RELATIONSHIP_CATEGORIES = [
  { label: "Works at", color: "#595959", match: /\b(works? at|employ|joined|hired)/i },
  { label: "Communicated with", color: "#737373", match: /\b(email|communicat|repl|wrote|message)/i },
  { label: "Vendor of", color: "#333333", match: /\b(vendor|supplie[rd]|contract)/i },
  { label: "Asked to process", color: "#000000", match: /\b(asked|instructed|requested)/i },
  { label: "Payment to", color: "#1a1a1a", match: /\b(payment|paid|invoice|transfer)/i },
];
...
const SERIES = ["#1a1a1a", "#595959", "#333333", "#737373", "#000000", "#999999"];
...
/** Radial score ring. */
function ringHTML(pct, label, note, color = "var(--brass, #1a1a1a)") {
...
function renderRelationshipList(edges) {
  ...
  const colorFor = label => (RELATIONSHIP_CATEGORIES.find(c => c.label === label) || { color: "#999999" }).color;
...
function renderGauge(score) {
  ...
  el.innerHTML = `
    <svg viewBox="0 0 200 118" class="gauge-svg">
      ...
      <path d="M20,100 A80,80 0 0 1 180,100" style="fill:none;stroke:var(--verdigris, #333333)" stroke-width="14" stroke-linecap="round" pathLength="100" stroke-dasharray="${score} 100"/>
    </svg>
...
function renderActivityChart(docs) {
  ...
  const dots = points.map(([x, y], i) => `
    <circle cx="${x.toFixed(1)}" cy="${y.toFixed(1)}" r="2.6" style="fill:var(--brass, #1a1a1a)">
    ...
    <svg viewBox="0 0 ${W} ${H}" class="activity-svg">
      <defs>
        <linearGradient id="activityFill" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" style="stop-color:var(--brass, #1a1a1a);stop-opacity:0.28"/>
          <stop offset="100%" style="stop-color:var(--brass, #1a1a1a);stop-opacity:0"/>
        </linearGradient>
      </defs>
      <line x1="${PAD}" y1="${H - PAD}" x2="${W - PAD}" y2="${H - PAD}" style="stroke:var(--line, #d9d9d9)" stroke-width="1"/>
      <path d="${areaPath}" fill="url(#activityFill)"/>
      <path d="${linePath}" style="fill:none;stroke:var(--brass, #1a1a1a)" stroke-width="2"/>
      ${dots}
    </svg>
...
const TIMELINE_CATEGORIES = [
  { key: "employment", label: "Employment", icon: "icon-briefcase", color: "#595959", match: /\b(began working|joined|hired|appointed|resigned|terminated|employ)/i },
  { key: "vendor", label: "Vendor / Org", icon: "icon-bank", color: "#333333", match: /\b(vendor|approved|contract|agreement|registered|onboard)/i },
  { key: "communication", label: "Communication", icon: "icon-mail", color: "#737373", match: /\b(emailed|e-mailed|wrote to|message|contacted)/i },
  { key: "reply", label: "Reply / Statement", icon: "icon-person", color: "#1a1a1a", match: /\b(replied|responded|stated|acknowledg|confirmed)/i },
  { key: "delivery", label: "Delivery", icon: "icon-truck", color: "#8c8c8c", match: /\b(deliver|shipment|warehouse|dispatch|received goods)/i },
  { key: "financial", label: "Financial", icon: "icon-invoice", color: "#000000", match: /\b(invoice|payment|paid|transfer|amount|inr|₹|rs\.)/i },
  { key: "task", label: "Task / Instruction", icon: "icon-doc-check", color: "#999999", match: /\b(asked|requested|instructed|process|task)/i },
];
function classifyEvent(description) {
  const found = TIMELINE_CATEGORIES.find(c => c.match.test(description || ""));
  return found || { key: "other", label: "Other", icon: "icon-clock", color: "#999999" };
}
...
const typeColors = { person: "#a6a6a6", organization: "#8c8c8c", location: "#bfbfbf", other: "#999999" };
const EDGE_CATEGORIES = [
  { key: "employment", label: "Employment", color: "#333333", match: /\b(works? at|employ|joined|hired)/i },
  { key: "business", label: "Business", color: "#595959", match: /\b(vendor|supplie[rd]|contract|business|payment|invoice)/i },
  { key: "communication", label: "Communication", color: "#000000", match: /\b(email|communicat|repl(y|ied)|wrote|message|requested|asked)/i },
  { key: "consultation", label: "Consultation", color: "#737373", match: /\b(consult|advis|shared info)/i },
];
function classifyEdge(relation) {
  const found = EDGE_CATEGORIES.find(c => c.match.test(relation || ""));
  return found || { key: "other", label: "Other", color: "#999999" };
}
...
const typeGlyphs = {
  ...
  person: '<circle cx="30" cy="23" r="9"/><path d="M12 50c2-11 9-17 18-17s16 6 18 17" fill="none" stroke="#0a0a0a" stroke-width="4.5" stroke-linecap="round"/>',
...
function nodeIcon(type) {
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="60" height="60"><circle cx="30" cy="30" r="28" fill="${color}" stroke="#333333" stroke-width="2"/><g fill="#0a0a0a">${glyph}</g></svg>`;
...
async function loadGraph() {
  ...
  GRAPH_DATA = data;
  GRAPH_DOCS = docs;
  legend.innerHTML = EDGE_CATEGORIES.concat([{ key: "other", label: "Other", color: "#999999" }])
...
function renderGraph() {
  ...
  const nodes = new vis.DataSet(visibleNodes.map(n => ({
    ...
    font: { color: "#0a0a0a", face: "Inter", size: 13, weight: 700, strokeWidth: 4, strokeColor: "#ffffff", vadjust: -30 },
  })));
  const edges = new vis.DataSet(visibleEdges.map((e, i) => {
    ...
    return {
      ...
      color: { color: cat.color, highlight: "#000000", hover: cat.color }, opacity: 0.85,
...
function contraSeverity(c) {
  ...
  if (!Number.isFinite(conf)) return { key: "low", label: "Low", color: "var(--steel, #666666)" };
  if (conf >= 75) return { key: "high", label: "High", color: "var(--crimson, #000000)" };
  if (conf >= 45) return { key: "med", label: "Medium", color: "var(--brass, #1a1a1a)" };
  return { key: "low", label: "Low", color: "var(--steel, #666666)" };
}
...
function renderContradictionCharts() {
  ...
  sevChart.innerHTML = list.length
    ? ringHTML(avgConf, "Average confidence",
        ...
        sevCounts.high ? "var(--crimson, #000000)" : "var(--brass, #1a1a1a)")
      + barChartHTML([
          { label: "High", value: sevCounts.high, color: "var(--crimson, #000000)" },
          { label: "Medium", value: sevCounts.med, color: "var(--brass, #1a1a1a)" },
          { label: "Low", value: sevCounts.low, color: "var(--steel, #666666)" },
        ])
  ...
  const docRows = Object.entries(docCounts).sort((a, b) => b[1] - a[1]).slice(0, 8)
    .map(([label, value]) => ({ label, value, color: "var(--crimson, #000000)" }));
    ...
    if (chartCard) {
      ...
      document.getElementById("similar-chart").innerHTML = barChartHTML(
        data.matches.slice().sort((a, b) => b.similarity - a.similarity).map(m => ({
          ...
          value: m.similarity,
          color: m.similarity >= 75 ? "var(--brass, #1a1a1a)" : m.similarity >= 50 ? "var(--steel, #666666)" : "var(--text-faint, #8c8c8c)",
        })), { suffix: "%" });
