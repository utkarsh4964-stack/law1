...
const RELATIONSHIP_CATEGORIES = [
  { label: "Works at", color: "#3b82f6", match: /\b(works? at|employ|joined|hired)/i },
...
function renderRelationshipList(edges) {
  ...
  const colorFor = label => (RELATIONSHIP_CATEGORIES.find(c => c.label === label) || { color: "#64748b" }).color;
...
function renderGauge(score) {
  ...
  el.innerHTML = `
    <svg viewBox="0 0 200 118" class="gauge-svg">
      ...
      <path d="M20,100 A80,80 0 0 1 180,100" style="fill:none;stroke:var(--verdigris, #18a862)" stroke-width="14" stroke-linecap="round" pathLength="100" stroke-dasharray="${score} 100"/>
    </svg>
...
function classifyEvent(description) {
  ...
  return found || { key: "other", label: "Other", icon: "icon-clock", color: "#64748b" };
}
...
const EDGE_CATEGORIES = [
  ...
  { key: "consultation", label: "Consultation", color: "#8b5cf6", match: /\b(consult|advis|shared info)/i },
];
function classifyEdge(relation) {
  const found = EDGE_CATEGORIES.find(c => c.match.test(relation || ""));
  return found || { key: "other", label: "Other", color: "#64748b" };
}
...
async function loadGraph() {
  ...
  GRAPH_DATA = data;
  GRAPH_DOCS = docs;
  legend.innerHTML = EDGE_CATEGORIES.concat([{ key: "other", label: "Other", color: "#64748b" }])
